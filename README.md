...
Leave `DD_MCP_URL` empty. The tool invokes the `datadog_mcp_cli` binary, which uses OAuth via browser. Install it from the [Datadog MCP docs](https://docs.datadoghq.com/bits_ai/mcp_server/).
...
The `knowledge/` directory contains markdown files searched by the `search_known_issues` tool:
